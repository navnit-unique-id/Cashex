package com.inspirado.kuber.util;
...
public class Util implements Serializable {
    ...
    public static void updateStoreInSharedPref(SharedPreferences pref, JSONObject response) {
        SharedPreferences.Editor editor = pref.edit();
        editor.putString("store", response.toString());
        editor.commit();
    }

    public static Store getStoreFromSharedPref(Context context) {
        SharedPreferences pref = context.getSharedPreferences("pref", 0);
        String json = pref.getString("store", "");
        return (new Gson()).fromJson(json, Store.class);
    }
